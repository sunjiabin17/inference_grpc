...
Engine::Engine(const std::string& model_path, const std::string& device) :
    _engine(nullptr),
    _context(nullptr),
    _runtime(nullptr),
    _logger(Logger()),
...
bool Engine::load_network(const std::string& enginefile) {
    ...
    std::shared_ptr<nvinfer1::IRuntime> runtime{nvinfer1::createInferRuntime(_logger)};
    ...
    std::shared_ptr<nvinfer1::ICudaEngine> engine{runtime->deserializeCudaEngine(buffer.data(), size)};
    if (engine == nullptr) {
    ...
    std::shared_ptr<nvinfer1::IExecutionContext> context{engine->createExecutionContext()};
    if (context == nullptr) {
...
int main(int argc, char** argv) {
    ...
    // std::unique_ptr<Engine> engine = std::make_unique<Engine>(model_path, device);
    Engine* engine = new Engine(model_path, device);
    // engine->compile("tf32", 1, model_path, "model.engine");
    engine->load_network("model.engine");
